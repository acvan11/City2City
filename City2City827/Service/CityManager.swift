...
final class CityManager {
    ...
    */
    ...
    //MARK: Only Save most recent 10 Cities (Query CoreData Results), No Duplicate Cities should be saved (Predicate)
    
    func corePredicateExample() {
        
        let fetchRequest = NSFetchRequest<CoreCity>(entityName: "CoreCity")
        let predicate = NSPredicate(format: "name==%@", "Atlanta") //string interpolation, for CoreData
        fetchRequest.predicate = predicate
        
        do {
            let coreCities = try context.fetch(fetchRequest)
            print(coreCities.count)
        } catch {
